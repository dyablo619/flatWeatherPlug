...
;(function ( $, window, document, undefined ) {
		...
		var apiurls = {
			...
			"openweathermap" : ["https://api.openweathermap.org/data/2.5/onecall"],
		...
		$.extend(Plugin.prototype, {
			...
			fetchWeather: function () {
				...
				if (this.settings.api == "openweathermap") {
					...
					var parameters = {};
					parameters.lat = this.settings.locLat;
					parameters.lon = this.settings.locLon
					parameters.exclude = "hourly,minutely";
		    	...
		    	.done(function(){
					...
					if (that.settings.api == "openweathermap" && !(args[1] == "success")) {
			...
			render : function (weather, context) {
				...
				if (context.settings.view != "simple") {
					...
					if (context.settings.view != "partial") {
						...
						$("<li/>", {"class" : "wi wind"}).text(formatWind(weather.today.wind_speed, weather.today.wind_deg, context.settings.units)).appendTo(atmo);
		...
		*/
		function datamapper (input, settings) {
			...
			if (settings.api == "openweathermap") {
				...
				out.today = {};
				out.today.temp = {};
				out.today.temp.now = Math.round(input[0].current.temp);
				out.today.temp.min = Math.round(input[0].daily[0].temp.min);
				out.today.temp.max = Math.round(input[0].daily[0].temp.max);

				out.today.desc = input[0].current.weather[0].description.capitalize();
				out.today.code = input[0].current.weather[0].id;
				...
				console.log(input[0].wind);

				out.today.wind = input[0].wind;
				out.today.wind_speed = input[0].current.wind_speed;
				out.today.wind_deg = input[0].current.wind_deg;
				out.today.humidity = input[0].current.humidity;
				out.today.pressure = input[0].current.pressure;
				out.today.sunrise = epochToHours(input[0].current.sunrise);
				out.today.sunset = epochToHours(input[0].current.sunset);
				...
				for (var i = 0; i < settings.forecast; i++) {
					var forecast = {};
					forecast.day = getDayString(new Date(input[0].daily[i].dt * 1000)); //api time is in unix epoch
					forecast.code = input[0].daily[i].weather[0].id;
					forecast.desc = input[0].daily[i].weather[0].description.capitalize();
					forecast.temp = {max: Math.round(input[0].daily[i].temp.max), min: Math.round(input[0].daily[i].temp.min)}
